...
public class Gb {
	...
	private static native void startEmulator(long gb, String rom, boolean debug, int rtcOffset);
	
	public void startEmulator(String rom) {
		startEmulator(gb, rom, false, 0);
	}
	
	public void startEmulator(String rom, int rtcOffset) {
		startEmulator(gb, rom, false, rtcOffset);
	}
	
	public void startEmulator(String rom, boolean debug) {
		startEmulator(gb, rom, debug, 0);
	}
	
	public void startEmulator(String rom, boolean debug, int rtcOffset) {
		startEmulator(gb, rom, debug, rtcOffset);
	}
	
	public void startEmulator(String rom, int rtcOffset, boolean debug) {
		startEmulator(gb, rom, debug, rtcOffset);
	}
...
//  private static final int MAX_SAVE_SIZE = 186675; // PokeRed
	...
	private static native int getCycleCount(long gb);
	
	public int getCycleCount() {
		return getCycleCount(gb);
	}
	...
	public String getEonTimer() {
		return String.format("%.3f", getGbpTime());
	}
	
	public String getLiveSplit() {
		return String.format("%.2f", getGbpTime());
	}
