...
public class Map {
	...
	private static Map[] maps = new Map[65536];
	...
	static {
		...
		ROUTE_29.setConnections(null, null, null, CHERRY_GROVE);
		CHERRY_GROVE.setConnections(ROUTE_30, ROUTE_29, null, null);
	}
	 ...
	 *
	 * @param x - pokeworld coord x
	 * @param y - pokeworld coord y
	...
	/**
	 * @param id    - the map id
	 * @param width - the map width in blocks
	 * @param path  - the path to a binary representation of the map used to
	...
	private Map(int id, int width, int pokeworldOffsetX, int pokeworldOffsetY, int globalEncounterRate, String path) {
		try {
			...
			if(id == 59) {
				...
				tiles[34][32].setCanMoveRight(false);
				...
				tiles[35][24].setCanMoveRight(false);
				tiles[24][32].setCanMoveRight(false);
				...
				tiles[35][24].setCanMoveLeft(false);
				tiles[35][23].setCanMoveLeft(false);
			...
			if(id == 59) {
				tiles[35][31] = new Tile(this, 35, 31, (byte)240);
				tiles[36][23] = new Tile(this, 36, 23, (byte)240);
				overworldTiles[35][31] = new OverworldTile(id, 35, 31, false);
				overworldTiles[36][23] = new OverworldTile(id, 36, 23, false);
				tiles[36][24].setCanMoveUp(true);
				tiles[35][32].setCanMoveUp(true);
			}
		} catch(Exception e) {
			e.printStackTrace();
		}
